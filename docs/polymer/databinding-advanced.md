...
In general, you shouldn’t need to manually mutate the DOM nodes generated by template bindings --
you can do most things you need to do simply by setting up bindings and mutating the model object.

If you need to mutate the DOM nodes generated by a template, it is safe to do so as long as you
...
Because the template identifies each instance by the terminating node, mutation of the template
instance’s state persists even if the template has to reorder its template instances.
...
{% raw %}
    <body is="polymer-body" unresolved>
      ...
      <script>
        window.model = {
          quotes: [ { attribution: "Plautus", 
              quote: "Let deeds match words."},
            { attribution: "Groucho Marx", quote: 
