...
**Important Note.** starting with Polymer Version 2.2.1 the \<custom-style\> Tag together with \<style is="custom-style" include="..."\> causing an error with 'Failed to execute 'insertBefore' on 'Node''. Don't use hybrid mode with newer Polymer versions. 
