...
Two-way binding to the values of native input elements is not supported in 0.8. Two-way binding is only supported on properties of Polymer elements that have two-way binding enabled (`notify: true`).

To get the value from a native input element, you can add an event listener to the `input` or `change` events. For example, the following simple element wraps an input and provides two bindable properties, `currentValue` and `committedValue`:

    <dom-module id="my-input">
  	  <template>
        <input id="input" type$="[[type]]" on-input="updateCurrent"  on-change="updateCommitted">
      </template>
    </dom-module>
    <script>
      Polymer({
