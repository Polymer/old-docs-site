...
- [Data binding](/docs/polymer/databinding.html)
...
2. Register your custom element using `Polymer()`.
...
**my-element.html**
    ...
    <dom-module id="my-element">
      <template>
        <span>Hello from <b>my-element</b>. This is my local DOM.</span>
      </template>
    </dom-module>
    <script>
    Polymer({ 
      ...
      is: "my-element" // this is my tag name
    });
    </script>

Two items to notice:

*   The `<dom-module>` element supplies a template for the element's [_local DOM_](). 

*   The `Polymer` method registers a new custom element. 

*   The `is` property is required. It specifies the name of the HTML tag you'll instantiate 
    in markup (in this case `<my-element>`). 
    and **must** contain a "-"
...
When you're creating a new element, you'll often need to expose a [public API](/docs/polymer/polymer.html#published-properties) so users can configure it. 
